		...
		visitor: {
			ExportNamedDeclaration(path, { opts: { loaderMap } }) {
				...
				replaceMatchingSpecifiers(path, loaderMap, (specifier, sourcePath) => {
					...
					return t.exportNamedDeclaration(
						null,
						[t.exportSpecifier(t.identifier(specifier.local.name), t.identifier(specifier.exported.name))],
						t.stringLiteral(generateNewSourcePath(loaderMap, specifier.local.name, sourcePath))
					);
				});
			},
			ImportDeclaration(path, { opts: { loaderMap } }) {
				replaceMatchingSpecifiers(path, loaderMap, (specifier, sourcePath) => {
					...
					return t.importDeclaration(
						[t.importSpecifier(t.identifier(specifier.local.name), t.identifier(specifier.imported.name))],
						t.stringLiteral(generateNewSourcePath(loaderMap, specifier.imported.name, sourcePath))
					);
				});
			}
		}
